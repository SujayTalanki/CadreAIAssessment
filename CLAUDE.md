...
- **`backend/`** — FastAPI app. Retrieval-augmented generation over a static FAQ corpus:
  ...
  2. Each `POST /api/chat` call embeds the latest user message and retrieves **the entire FAQ corpus** (`k=collection.count()`, not a fixed number), appending all of it to the static system prompt (`app/system_prompt.py`) before calling Claude. This isn't a bug - see the comment in `llm_client.py`. Fixed top-k values (3, 5, 6, 10) each got outpaced by corpus growth in turn, as compound questions repeatedly pushed a relevant chunk out of the window; embedding similarity isn't reliable at distinguishing short, closely-related FAQ entries at this scale. With the corpus this small, always including everything is simpler and permanently removes that failure mode. Revisit top-k filtering (with a real chunking/re-ranking strategy) only once the corpus is large enough that including it all becomes a real cost/latency concern.

...
- **Knowledge base: RAG, not a giant system prompt.** FAQs live in `backend/app/data/faqs.json` (static, human-editable) and are embedded into an in-memory Chroma collection on startup, using Chroma's bundled default embedding function — no external embeddings API key required. `retrieval.query()` is a standard top-k retrieval function - it takes an explicit `k` and always did. What changed is the `k` we pass to it at the `/api/chat` call site: retrieval was tuned repeatedly during live testing, with fixed k values (3 → 5 → 6 → 10) each getting outpaced as the corpus grew from 10 to 15 entries, as compound questions ("what do you do, and do you work with real estate?") repeatedly pushed the `industries-served` chunk out of the window. Embedding similarity isn't reliable at distinguishing short, closely-related FAQ entries at this scale, so top-k filtering wasn't actually earning its keep yet - we now pass `k=collection.count()`, retrieving the entire corpus every turn, which is simpler and permanently removes the failure mode at negligible extra token cost given how small the corpus still is (27 entries as of this writing). This is a knob, not a wall: if the corpus grows enough that whole-corpus retrieval becomes a real cost/latency concern, reverting to a fixed `k` (with a real re-ranking strategy this time) is a one-line change at the call site, not new infrastructure. This also means the FAQ set is extensible today (add a JSON entry, redeploy - no prompt surgery) and gives the system a real retrieval/data-model story instead of one unbounded prompt.
...
- **No re-ranking, hybrid search, or similarity-threshold-based escalation logic** — the whole corpus plus the model's own judgment is sufficient at this corpus size; top-k filtering isn't needed yet.
